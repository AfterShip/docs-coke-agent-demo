...
type Options struct {
	...
	Level             string   `json:"level"              mapstructure:"level" validate:"omitempty,oneof=debug info warn error dpanic panic fatal"`
	Format            string   `json:"format"             mapstructure:"format" validate:"omitempty,oneof=console json"`
...
func (o *Options) Validate() error {
	...
	if err := zapLevel.UnmarshalText([]byte(o.Level)); err != nil {
		errs = append(errs, fmt.Sprintf("unrecognized level: %q", o.Level))
	}

	// Validate format
	if o.Format != consoleFormat && o.Format != jsonFormat {
		errs = append(errs, fmt.Sprintf("not a valid log format: %q", o.Format))
	}
